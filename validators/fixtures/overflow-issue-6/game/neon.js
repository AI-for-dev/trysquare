...
//
// Collision detection is handled by the pure, exported brickHit() function,
// called from the game loop. Rendering lives in frame(), kept separate.
//
...
// Simple AABB test: it only says WHETHER the ball and the brick overlap, not
// WHICH face was hit. The missing bounce in frame() is what lets the ball plow
// straight through bricks (see #1).
...
// Returns the first alive brick the ball overlaps, or null.
// Pure, testable – no side effects, no rendering code.
export function brickHit(ball, bricks) {
...
// the walls and the paddle. Brick collisions are handled separately via
// brickHit() in the game loop.
...
// --- Rendering (pure draw pass, no collision code) -------------------------

// Renders the field. Collision detection is handled separately in the game
// loop (via the pure brickHit function), not in here.
function frame(ctx, state) {
  // Background
  ctx.fillStyle = palette.background;
  ctx.fillRect(0, 0, WIDTH, HEIGHT);

  // Bricks (rendering only)
...
export function boot(doc = document) {
  ...
  function loop(now) {
    ...
    if (state.running) {
      ...
      // Brick collision (separate from rendering)
      const hit = brickHit(state.ball, state.bricks);
      if (hit) {
        hit.alive = false;
        state.combo += 1;
        state.score += scoreForBrick(hit, state.combo);
      }
