...
// Helper to check collisions with bricks using grid lookup.
function handleBrickCollisions(state) {
  const { ball, bricks } = state;

  const startCol = Math.floor((ball.x - ball.r - BRICK_OFFSET_X) / (BRICK_W + BRICK_GAP));
  const endCol = Math.floor((ball.x + ball.r - BRICK_OFFSET_X) / (BRICK_W + BRICK_GAP));
  const startRow = Math.floor((ball.y - ball.r - BRICK_TOP) / (BRICK_H + BRICK_GAP));
  const endRow = Math.floor((ball.y + ball.r - BRICK_TOP) / (BRICK_H + BRICK_GAP));

  for (let row = Math.max(0, startRow); row <= Math.min(BRICK_ROWS - 1, endRow); row++) {
    for (let col = Math.max(0, startCol); col <= Math.min(BRICK_COLS - 1, endCol); col++) {
      const brick = bricks[row * BRICK_COLS + col];
      if (brick && brick.alive && collides(ball, brick)) {
        brick.alive = false;
        state.combo += 1;
...
// the walls, the paddle, and bricks.
